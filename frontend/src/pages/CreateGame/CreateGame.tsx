...
const schema = z.object({
  ...
  identificator: z
    .string()
    .trim()
    .min(10, { message: "Минимальная длина 10 символа" })
    .max(10, { message: "Максимальная длина 10 символов" }),
  price: z.coerce
    .number()
    .positive({ message: "Должен быть больше 0" })
    .optional(),
});
      ...
      <form onSubmit={handleSubmit(postData)} className="w-full flex flex-col">
        ...
        <div className="mb-2">
          ...
          <input
            ...
            type="text"
            id="identificator"
            className="w-full border-[#C0E3E5] solid border-[2.8px] rounded-[20px] px-7 py-3 text-[#333333] text-2xl appearance-none"
          />
        </div>
        <p className="mb-6 text-[#979797] text-[10px] font-bold leading-[34px] text-center">
          Придумайте уникальный идентификатор для коробки, который будет
          указываться в ссылке
        </p>
        <div className="flex justify-between items-center w-full border-[#C0E3E5] solid border-[2.8px] rounded-[20px] px-7 py-3">
          <input
            readOnly
            placeholder="Максимальная стоимость подарка"
            type="text"
            id="identificator"
            className="w-full placeholder:text-[#333333] text-[#333333] text-2xl appearance-none cursor-default"
          />
          <label
        ...
        <p className="mb-16 text-[#979797] text-[10px] font-bold leading-[34px] text-center">
          При включенной опции участникам будет показано ограничение, которому
          они должны будут следовать
        </p>

        {checked && (
          <div
            className={`w-[400px] self-center py-4 px-7 bg-white border-[#C0E3E5] ${
              errors.price ? "border-[#FF0000]" : ""
            }  $ border-[2px] solid rounded-[34px] mb-14`}
          >
            <input
              type="number"
              id="price"
              {...register("price")}
              placeholder="Укажите максимальную стоимость подарка"
              className={`w-full border-none bg-transparent focus-visible:outline-none text-center text-[#333333] text-2xl placeholder:text-sm placeholder:font-bold ${
                errors.price ? "placeholder:text-[#BE081E]" : ""
              }`}
            />
          </div>
        )}
